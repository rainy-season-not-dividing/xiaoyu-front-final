...
import { cancelLikePost } from '@/api/posts'
import { useUserStore } from '@/stores'
import { timeAgo } from '@/utils/timeFormat'
import { getSelfAllLisk } from '@/api/user'
...
//正式
onMounted(async () => {
    const { data: { data } } = await getSelfAllLisk();
...
const deleteArticleConfirm = async () => {
    console.log("确定取消点赞该文章:");
    //发送删除请求,取消收藏id就是deleteId.value
    //刷新页面


    await cancelLikePost(deleteId.value);
...
<template>
    ...
    <van-sticky class="sticky-component">
        <van-nav-bar title="我的喜欢" left-text="" :right-text="isManaging ? '取消' : '管理'" left-arrow
    ...
    <div class="main">
        <div class="article" v-for="item in form" :key="item.id" @click="goDetail(item.id)">
            ...
            <div class="actions" :style="{ opacity: isManaging ? 1 : 0 }">
                <van-button class="del" @click.stop="deleteArticle(item.id, $event)">取消点赞</van-button>
            </div>
        </div>
    </div>
