...
import { getSelfAllPost, deleteSelfPost } from '@/api/posts'
...
onMounted(async()=>
 {
    const { data: { data } } = await getSelfAllPost(userId);
...
//获得文章数据json对象
  ...
  {   
    console.log("确定删除文章:");
    //发送删除请求,删除文章id就是deleteId.value
    //刷新页面
    await deleteSelfPost(deleteId.value);
    ...
    <div class="main">
        <div class="article" v-for="item in form" :key="item.id" @click="goDetail(item.id)">
          ...
          <div class="actions" :style="{ opacity: isManaging ? 1 : 0 }">
            <van-button  class="edit" @click.stop="edit(item.id)">编辑</van-button>
            <van-button  class="del" @click.stop="deleteArticle(item.id,$event)">删除</van-button>
          </div>
        </div>
    </div>
...
.sticky-component
{
    ...
    .article
    {
      ...
      transition: all .3s;
      opacity: 1;
      display: flex; /* 让文章内容和操作区可以更好地布局 */
      padding-left: 20px; /* 给文章文字留点空间 */
      box-shadow: 5px 4px 15px rgba(0, 0, 0, 0.2); /* 优化阴影 */
    }
    .article:hover
    {
